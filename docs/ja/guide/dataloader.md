...
`GlyphSample` はグリフ参照と target index を持ちます。pipeline の最初に
`LoadGlyph` を使うと、sample metadata を保持したまま意味型 `Outline` を読み込めます。

`GlyphDataset` には、PyTorch の Dataset と同様にアイテムごとに変換を適用する
